...
export const Sidebar: React.FC<SidebarProps> = ({ isCollapsed, setIsCollapsed }) => {
  ...
  return (
    ...
    >
      {/* Brand Header */}
      <div className={`flex items-center h-16 shrink-0 border-b border-[#334155]/60 transition-all duration-200 ${isCollapsed ? 'justify-center px-0' : 'justify-between px-4'}`}>
        {!isCollapsed ? (
          <>
          ...
          <div className="flex flex-col items-center justify-center w-full h-full relative group cursor-pointer" onClick={() => setIsCollapsed(false)} aria-label="Expand sidebar">
            <div className="w-10 h-10 flex items-center justify-center shrink-0 transition-transform group-hover:scale-105">
              <LogoIcon size={40} />
            </div>
            <div className="absolute -right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-[#1E293B] rounded-full p-0.5 shadow-md border border-[#334155]">
              <ChevronRight className="h-3 w-3 text-[#7DF5F5]" />
            </div>
          </div>
        )}
      </div>
